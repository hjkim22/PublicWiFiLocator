...
<form action="getWifi.jsp" method="post">
    ...
    </label>    <input type="button" id="pos_btn" value="내 위치 가져오기" onclick="getPosition()">
    <input type="submit" value="근처 WIPI 정보 보기">
</form>

<table>
    <thead>    <tr>        <th>거리(Km)</th>
        <th>관리번호</th>
        <th>자치구</th>
        <th>와이파이명</th>
        <th>도로명주소</th>
...
%>
<tr>
    <td>            <%=list.get(i).getDistance()%>
    </td>
    <td>            <%=list.get(i).getX_SWIFI_MGR_NO()%>
    </td>
    <td>            <%=list.get(i).getX_SWIFI_WRDOFC()%>
    </td>
    <td>            <a href="detail.jsp?distance=<%=list.get(i).getDistance()%>&mgrNo=<%=list.get(i).getX_SWIFI_MGR_NO()%>"><%=list.get(i).getX_SWIFI_MAIN_NM()%>
    </a>
    </td>        <td>            <%=list.get(i).getX_SWIFI_ADRES1()%>
</td>
